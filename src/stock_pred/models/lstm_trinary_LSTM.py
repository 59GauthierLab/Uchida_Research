...
from tensorflow.keras.layers import Input, Dense, Dropout, LayerNormalization, MultiHeadAttention, GlobalAveragePooling1D, Lambda, LSTM, Bidirectional
...
# logs (to match trinary_transformer_old.py)
import os
import json
import platform
...
LOGCFG = LogConfig(
    quiet_fit=True,
    save_json=True,
    json_path="LSTM/run_summary.json",
...
def make_cosine_lr_callback(base_lr: float, alpha: float, total_epochs: int):
    ...
    def _schedule(epoch, lr):
        ...
        return float(new_lr)
    return tf.keras.callbacks.LearningRateScheduler(_schedule, verbose=1)
...
# ---------- LSTM ----------
def lstm_model(win: int, fdim: int, n_classes: int) -> tf.keras.Model:
    """
    LSTMベースのシンプルな時系列分類モデル。
    - 入力: (B, T=win, F=fdim)
    - 構成: BiLSTM(return_sequences=True) ×2 → プーリング(avg/last) → Dropout → Dense
    - ハイパラは既存を流用: 単位数=MODEL.ff_dim, dropout=MODEL.dropout
    """
    inp = Input(shape=(win, fdim))

    x = Bidirectional(LSTM(MODEL.ff_dim, return_sequences=True, dropout=MODEL.dropout),
                      name="bilstm_1")(inp)
    x = Bidirectional(LSTM(MODEL.ff_dim, return_sequences=True, dropout=MODEL.dropout),
                      name="bilstm_2")(x)
    ...
    m = Model(inp, out)
    # 期待入力形状とモデルサマリも出力（transformer_old と同等の可視性）
    try:
...
def main():
    ...
    # --- run summary (JSON) to match trinary_transformer_old.py ---
    run_summary = {
    ...
    for fold, (tr_idx, va_idx) in enumerate(splits, 1):
        ...
        # === 入力形状ログ（B,T,F）と一貫性チェック ===
        _log_btf("Xtr_win", Xtr_win)
        _log_btf("Xiv_win",  Xiv_win)
        _log_btf("Xva_win",  Xva_win)
        assert Xtr_win.shape[1] == DATA.win, f"win(T) mismatch: {Xtr_win.shape[1]} vs DATA.win={DATA.win}"
        assert Xiv_win.shape[-1] == Xtr_win.shape[-1] == Xva_win.shape[-1], \
            f"fdim mismatch: tr={Xtr_win.shape[-1]}, iv={Xiv_win.shape[-1]}, va={Xva_win.shape[-1]}"
        print(f"[SHAPE] fdim(F)={Xtr_win.shape[-1]}, win(T)={DATA.win}")

        Xtr_inn_win = Xtr_win
        ytr_inn_win = ytr_win

        n_classes = 3 if TASK == "trinary" else 2
        model = lstm_model(DATA.win, Xtr_win.shape[-1], n_classes)
        ...
        if TRAIN.es_tune and (fold == 1):
            ...
            for p in TRAIN.es_patience_grid:
                for d in TRAIN.es_mindelta_grid:
                    ...
                    tmp_model = lstm_model(DATA.win, Xtr_win.shape[-1], n_classes)
        ...
        # --- fold record (for JSON) to match trinary_transformer_old.py ---
        fold_rec = {
            "fold": int(fold),
            "counts_raw": {
